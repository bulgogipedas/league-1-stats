  ...
  return (
    <div ref={ref} className="relative w-full overflow-hidden border border-border bg-pitch">
      <svg width={width} height={height} role="img" aria-label="Formation pitch" onMouseLeave={() => setTooltip(null)}>
        <rect x="12" y="12" width={width - 24} height={height - 24} fill="var(--pitch-green)" stroke="var(--pitch-lines)" strokeWidth="2" />
        <line x1={width / 2} x2={width / 2} y1="12" y2={height - 12} stroke="var(--pitch-lines)" />
        <circle cx={width / 2} cy={height / 2} r={height * 0.12} fill="none" stroke="var(--pitch-lines)" />
        <rect x="12" y={height * 0.29} width={width * 0.12} height={height * 0.42} fill="none" stroke="var(--pitch-lines)" />
        <rect x={width - 12 - width * 0.12} y={height * 0.29} width={width * 0.12} height={height * 0.42} fill="none" stroke="var(--pitch-lines)" />
        {squad.map((player, index) => (
          <g key={`${player.Player || index}-${index}`} transform={`translate(${sx(player.x)},${sy(player.y)})`}>
            <circle
              ...
              fill={positionColors[player.pos] || "var(--chart-blue)"}
              fillOpacity="0.9"
